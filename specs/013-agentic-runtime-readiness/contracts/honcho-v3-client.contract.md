...
| 404 on `/v3` path, 4xx schema mismatch, version mismatch | `permanent` | `logger.error({err,...},'honcho API mismatch')` + readiness/health flag + `honcho_degraded`; return empty/no-op |
| success | — | return data |
...
- **AC4**: pointed at a wrong/legacy API → `permanent` signal (loud), turn still completes. (FR-007)
