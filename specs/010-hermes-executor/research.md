...
- **Tools (MCP)**: external MCP server passed in `session/new.mcpServers` (`{type:'stdio', name, command, args, env:[{name,value}]}`); tool namespaced `mcp_<server>_<tool>`; `tools/call` routed to the engine server (**proven via out-of-band marker file**). **Tool results auto-fenced by hermes** in an `<untrusted_tool_result>` envelope — built-in prompt-injection defense (safety plus).
