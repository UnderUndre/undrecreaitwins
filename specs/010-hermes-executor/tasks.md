...
- [ ] T009 [BE] `agent-lifecycle.ts` — **ACP-backed** pool: spawn/`session/new` or **resume** (ACP `sessionCapabilities.resume/fork/list`) / hibernate (drop/keep session) / evict + warm-pool of **live `hermes acp` processes** (Redis registry, keyed **`(tenant,persona)`** — process per tenant+persona; that pair's conversations = ACP sessions *within*) **with documented default size** (load-bearing — claude F6; tuned T023). **Pool = process-per-(tenant,persona) with an isolated memory store/HOME** (T000d 🔴: memory durable+global+auto-injected, even tool-off); conversations of the same (tenant,persona) may share its process ONLY with native memory OFF + an isolated/empty per-process store (engine owns per-conversation memory); T000d showed in-process sessions otherwise bleed → else process-per-conversation.
