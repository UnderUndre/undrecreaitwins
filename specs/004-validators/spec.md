...
- **FR-008** (identity-guard) *(intent-level; blocked on recon R-identity)*: The system MUST detect replies that misrepresent the twin's identity or service provider relative to a persona-configured policy, and remediate them before delivery. The exact detection mechanism (deterministic vs. judge), the policy source, and the remediation action are **pending a focused reconnaissance of the legacy `identity-and-provider-guard`** and will be locked before this validator enters planning. [NEEDS CLARIFICATION: identity-guard mechanism + remediation — awaiting recon R-identity]
