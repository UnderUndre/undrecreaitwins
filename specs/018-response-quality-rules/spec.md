...
- Re-validation of rewrite output (1 pass through identity-guard + language-guard [017 seam] + conditional false-promise)
...
- **FR-002**: Per-assistant rule cache with TTL (default 60s, configurable via `CORRECTION_RULE_CACHE_TTL_MS`) + webhook invalidation (`POST /v1/internal/rules-reload`). **Webhook auth (CL Round 2)**: route MUST verify a dedicated shared secret `TWIN_INTERNAL_WEBHOOK_SECRET` (Bearer or HMAC over body), separate from `TWIN_PRODUCT_API_KEY`. Unauthenticated/invalid → `401`, cache untouched (closes DoS/cache-poison vector). Full snapshot re-pulled on next reply after invalidation. **Multi-instance note (review F4)**: cache is in-memory `Map` per process; webhook invalidates only the receiving instance. In multi-instance deploy, other instances serve stale rules until TTL (60s) expiry. Acceptable for Phase 1 (single-instance). Multi-instance → Redis pub/sub broadcast (Phase 2).
...
- **FR-007**: Re-validation — after rewrite, instantiate `FalsePromiseValidator(llm)` + `IdentityGuardValidator()` directly (004 modules), call `validateAndMutate(rewrittenText, context)` on each, and check the result for violations WITHOUT applying any text mutations. This detects violations (false-promise, identity-guard leak) introduced by the rewrite without mutating the re-validated text. 1 pass. No infinite loop. (Direct instantiation — not the shared `validatorPipeline.validateResponse()` singleton — because the singleton may apply remediation mutations; re-validation needs detection only.) **Conditional false-promise LLM (review F1)**: the false-promise LLM judge (~800ms) is SKIPPED when a cheap structural pre-check detects that the rewrite introduced no new promise-like tokens (new numerals, commitment phrases, price/guarantee language) vs the pre-DAR text. Identity-guard always runs (regex, ~0ms). This keeps the common rewrite (tone/style fix only) under the latency budget. **Format-injection exclusion (review F6)**: format-injection is an INPUT validator (strips prompt-injection patterns from user messages), NOT a response guard — it is not applicable to rewritten assistant replies. This is why it's excluded from re-validation, not an oversight. **Language-guard inclusion (017 seam)**: once `017-language-guard-validator` lands in the 004 family, re-validation ALSO runs `LanguageGuardValidator` (deterministic Unicode-range check, ~0 LLM) — a DAR rewrite can re-introduce off-language script (e.g., rewrite drifts into Chinese), which MUST be caught → rollback. Re-validation **response-guard set** = identity-guard + language-guard (both cheap, always run) + false-promise (conditional per review F1).
...
- **FR-009**: QualityEvent push client — `POST <PRODUCT_API_URL>/v1/quality-events` with Bearer auth + X-Tenant-ID. Body: `{ events: QualityEventPush[] }`. Fire-and-forget (errors logged, don't block reply delivery). **Idempotency (review F3)**: each event carries `idempotencyKey` = `${messageId}:${ruleId}:${attempt}`. Product MUST upsert on this key — if the 015 retry worker re-runs the turn and DAR fires again, `attempt` increments, preventing duplicate rows. **DAR re-runs on retry**: yes — if chat-service retries a turn (provider error → 015 worker), DAR re-executes on the retried reply. Each run gets a distinct `attempt` number in the idempotency key.
...
- **FR-013**: Latency budget — total DAR latency (detect + aggregate + rewrite + re-validate) p95 < 2s for the **score-only path** (structural detectors, zero LLM). **Rewrite path** (semantic detect + rewrite + conditional re-validation) may exceed 2s on p95 — worst-case ~2.4s serial LLM chain (review F1). Mitigation: conditional false-promise skip (FR-007) eliminates the LLM re-validation call on common tone-only rewrites. Hard timeout 5s per DAR turn (abort + fail-open). Budget overflow → skip lowest-priority score-mode semantic detectors. Structural detectors (regex/keyword) always run (0 LLM cost).
- **FR-014**: Error handling — if Product API is unavailable (pull fails), DAR is skipped (reply delivered without custom rules). If push fails, events are logged locally (pino) and dropped (no retry queue in Phase 1). Reply delivery never blocked by DAR failure. **Advisory vs compliance (review F7)**: 018 custom rules are **quality/tone** rules (advisory). Compliance-critical guarantees (false-promise, identity-guard) are in the **004 structural baseline**, which ALWAYS runs — they are never skipped by DAR failure. If a business needs a compliance-critical custom guard (e.g., "never quote competitor X"), it should be promoted to a 004 catalog validator, not a 019 custom rule. `enforcement: 'advisory' | 'required'` field on rules is a Phase 2 concern.
...
- **NFR-2 (perf)**: DAR total latency p95 < 2s for **score-only path** (structural detectors, zero LLM). **Rewrite path** may exceed 2s p95 (serial LLM chain: semantic detect + rewrite + conditional re-validation); hard timeout 5s per turn. Regex/keyword detection <1ms each. Semantic detection parallelized (cap ≤3 concurrent LLM calls). Rewrite = 1 LLM call. Re-validation: identity-guard always (regex, 0ms); false-promise LLM conditional (skipped when structural pre-check shows no new promise-like tokens — FR-007).
...
- **Regex pattern invalid (compiled at pull time? or rule creation?)** → Product validates at creation (400 BAD_REQUEST). Engine should still wrap `new RegExp()` in try/catch — if invalid pattern slips through, skip that rule, log error. **ReDoS (review F5)**: operator-authored regex patterns could contain catastrophic backtracking. Product should validate regex complexity at creation (reject patterns with nested quantifiers like `(a+)+`). Engine wraps regex execution defensively; future: switch to RE2 (linear-time regex) if ReDoS incidents occur.
...
- **Conflicting rewrite instructions (review F2)** → two rewrite rules with contradictory instructions ("be formal" vs "be casual") both land in the aggregated rewrite prompt. Resolved by **priority order**: the rewriter prompt explicitly instructs "follow instructions in priority order (highest first); if two conflict, obey the higher-priority one." Lower-priority conflicting instructions are NOT dropped (the LLM resolves); but priority ordering is the deterministic contract. If output is still inconsistent, operator sees it in calibration dashboard and adjusts priorities.
- **Operator instruction prompt-injection (review F8)** → `rewriteInstruction` is operator-authored free text injected into the rewriter LLM prompt. A careless/compromised operator could steer the rewriter. Mitigation: instructions are wrapped in a delimited block (`<operator_instructions>...</operator_instructions>`) in the prompt; the system instruction tells the LLM to treat them as editing constraints, not system commands. 004 re-validation (false-promise + identity-guard) is the backstop. Operator trust boundary = tenant admin role (same as all Product config).
...
- **QualityEventPush** (written to Product via HTTP): `{ assistantId, ruleId, ruleName, conversationId, messageId, mode, verdict, originalText?, rewrittenText?, score?, latencyMs, rolledBack, idempotencyKey, snapshotVersion }`. **`verdict` enum (CL Round 2)**: `'pass' | 'fail' | 'rewritten' | 'rolled_back' | 'overflow_skipped'` — canonical set (`overflow_skipped` = rule triggered but dropped by the ≤4 rewrite cap, FR-005). **`idempotencyKey` (review F3)** = `${messageId}:${ruleId}:${attempt}` — Product upserts. **`snapshotVersion` (review F9)** = rule-set snapshot version when event fired — for version attribution on the calibration dashboard.
- **RuleCacheEntry**: `{ rules: CorrectionRule[], snapshotVersion: string, fetchedAt: number }`.

## Dependencies

- **004-validators** (prerequisite): ValidatorPipeline must run BEFORE DAR. DAR re-validation instantiates 004's `FalsePromiseValidator` + `IdentityGuardValidator` directly (not the shared singleton) — detection only, no mutation.
- **017-language-guard-validator** (seam, soft-dependency): a 004-family response validator (deterministic, BLOCKING, 0-LLM). When present, DAR re-validation includes `LanguageGuardValidator` (FR-007) so a rewrite can't ship off-language script. 017 is NOT folded into 018 — it's a fixed structural guard in 004; 018 is the dynamic operator-rule layer above it.
- **Pipeline ordering contract (seam)**: `004 (false-promise → language-guard → identity-guard) → 018 DAR (detect → aggregate → rewrite) → re-validate via 004 response-guards (identity + language + conditional false-promise)`. Do NOT collapse 017 into the DAR layer — different trust/latency/failure semantics (fixed vs operator-authored; 0-LLM vs LLM-heavy; BLOCKING vs fail-open).
- **Shared verdict/audit vocabulary (seam A — resolved)**: 018 `verdict` enum (`pass|fail|rewritten|rolled_back|overflow_skipped`) maps to the canonical `QualityVerdict` union defined in 017 (owner: 004-family, module `packages/core/src/types/quality-event.ts`, task 017-T000). 018's `QualityEventPush` serializes from `QualityEvent`. 017-T000 is a hard dependency for 018 T005 (types). See 017 spec §Assumptions for the canonical type definition.
- **Shared internal-auth preHandler (seam B)**: 018 route `POST /v1/internal/rules-reload` and 019 route `GET /v1/internal/retrieved-feedback` both use a shared Fastify preHandler at `packages/api/src/middleware/internal-auth.ts` (verifies `TWIN_INTERNAL_WEBHOOK_SECRET` Bearer in one place). One secret, one verification site, route-scoped. Blast radius documented: one leak compromises both routes (same trust boundary = internal Product→Engine). Per-route secrets deferred to Phase 2.
- **Shared operator-text sanitization (seam C)**: 018 `rewriteInstruction` wrapping (tasks T011) and 019 `lesson` wrapping both use a shared util `wrapOperatorText()` at `packages/core/src/services/prompt-safety.ts`. Same delimiter (`<operator_instructions>`), same escape, same trust-boundary documentation.
...
- **Re-validation** — post-rewrite safety check through 004 response-guards: identity-guard + language-guard (017 seam) + conditional false-promise.
