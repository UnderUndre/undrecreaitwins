...
- [ ] T001 [DB] Create `tuning_drafts` drizzle schema in `packages/core/src/db/schema/tuning.ts` per data-model.md §1.4 — all columns, enums, indexes, RLS policy
- [ ] T002 [DB] Generate drizzle migration (`pnpm db:generate`) for `tuning_drafts` table
- [ ] T003 [SETUP] Define shared types in `packages/core/src/types/tuning.ts`: `TuningDraft`, `TuningDraftStatus`, `TuningMethod`, `ConfidenceLevel`, `TuningDraftWithDetails` — mapped from data-model.md §1–3
- [ ] T004 [BE] Implement `TuningDraftRepository` in `packages/core/src/services/tuning/tuning-draft-repository.ts` — CRUD operations: `create()`, `getById(tenantScoped)`, `listByPersona()`, `update()`, `getActiveDraft()`, `supersedeActiveDraft()` — all tenant-scoped via `withTenantContext`. Include concurrent-lock check: throw on second `generating` draft for same persona (FR-011).
- [ ] T005 [BE] Implement Redis helper module in `packages/core/src/services/tuning/redis-helper.ts` — connection, key builders (`interviewKey`, `proposalKey`, `proposalsKey`), set/get with TTL, delete. Fallback in-memory Map for dev (no Redis).
- [ ] T006 [SETUP] Create Zod validation schemas in `packages/api/src/schemas/tuning.ts`: `GenerateRequest`, `ReviewRequest`, `InterviewAnswerRequest`, `SandboxPreviewRequest`, `ProposalAcceptRequest` — per contract tuning-api.md
- [ ] T007 [BE] Create route scaffolding in `packages/api/src/routes/tuning/index.ts` — `fastify.register` entry that prefixes `/v1/personas/:personaId/tuning/*` and `/v1/tuning/*` routes. Wire auth + tenant middleware
...
- [ ] T008 [BE] [US1] Implement `DocExtractionPipeline` in `packages/core/src/services/tuning/doc-extraction-pipeline.ts` — reads RAG chunks for persona, calls LLM with extraction prompt (`response_format: json_object`), parses structured output, creates draft with status=ready. Handle: LLM timeout → status=failed LLM_TIMEOUT, unparseable JSON → partial draft systemPrompt only + confidence=low, chunk count validation
- [ ] T009 [BE] [US1] Implement generate route handler in `packages/api/src/routes/tuning/generate.ts` — validate persona exists + belongs to tenant, create draft (status=generating), fire-and-forget `process.nextTick(() => runGenerationPipeline(draftId, tenantId))`, return 202 `{ draftId, status: 'generating' }`. Validate method enum, check concurrent generate (FR-011) → 409 `CONFLICT_DRAFT_ACTIVE`. Validate persona has documents → 400 `NO_DOCUMENTS`
- [ ] T010 [BE] [US1] Implement poll draft route in `packages/api/src/routes/tuning/drafts.ts` — `GET /v1/tuning/drafts/:draftId` returns full draft object. Implement poll-time reaper (FR-003): if draft status=generating AND now - createdAt > 90s → flip to failed (GENERATION_STALLED), return failed draft. Tenant-scoped (FR-012).
...
- [ ] T012 [BE] [US1] Implement quality gate validator dry-run (FR-013) — after extraction pipeline completes, run `ValidatorPipeline.runDryRun()` on generated systemPrompt. If block-rate >30% → set confidence='low'
...
- [ ] T014 [BE] [US2] Implement `ActivatePipeline` in `packages/core/src/services/tuning/activate-pipeline.ts` — synchronously: (1) save previousSnapshot (current persona config via `PersonaRepository.getPersonaById`), (2) mark any currently-activated non-superseded draft for this persona as superseded, (3) update persona system_prompt + traits, (4) create funnel version via `FunnelRepository.createFunnelVersion()`, (5) set validator toggles, (6) mark draft activated (`status=activated`, `activatedAt=now()`). All in a single DB transaction. Validate draft belongs to tenant + persona (FR-012).
...
- [ ] T016 [BE] [US2] Implement rollback route in `packages/api/src/routes/tuning/rollback.ts` — `POST /v1/tuning/drafts/:draftId/rollback`. Restore previousSnapshot to live persona config (FR-007). Only works on current active draft (status=activated, not superseded) with non-null previousSnapshot. Superseded draft → 409 `DRAFT_SUPERSEDED`. No previousSnapshot → 400 `NO_PREVIOUS_SNAPSHOT`. Return 200 `{ status: 'rolled-back' }`.
...
- [ ] T018 [BE] [US5] Implement `DraftConfigOverlay` type and `SandboxDraftMode` service in `packages/core/src/services/tuning/sandbox-draft-mode.ts` — load draft config, create overlay object that shadows `systemPrompt`, `funnelConfig`, `validatorToggles`. Patch `ChatService.buildSystemPrompt()` to inject draft overlay when provided.
...
- [ ] T020 [BE] [US3] Implement `InterviewStateMachine` in `packages/core/src/services/tuning/interview-state-machine.ts` — manages interview session: `startSession(personaId)`, `getNextQuestion(personaId)` → returns question or draft if complete, `submitAnswer(personaId, questionId, answer)` → updates state. Question bank per research.md §3.1 (7 questions, Russian). Adaptive skipping per research.md §3.2 — check if persona has document chunks, skip questions already covered. Store state in Redis via `redis-helper.ts` (TTL 30min). When all questions answered → compile answers → call LLM extraction → create draft (method: 'interview') → return draftId.
...
- [ ] T023 [BE] [US4] Implement proposals routes in `packages/api/src/routes/tuning/proposals.ts` — `GET /v1/personas/:personaId/tuning/proposals` returns `{ proposals: TuningProposal[] }` with warmup indicator if <20 conversations. `POST /v1/tuning/proposals/:proposalId/accept` — resolve proposal from Redis cache, create draft with method='self-tuner', return `{ draftId }`. Cache miss → 404 `PROPOSAL_EXPIRED` (FR-010). `POST /v1/tuning/proposals/:proposalId/reject` — dismiss from cache, return `{ dismissed: true }`.
